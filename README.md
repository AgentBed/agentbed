...
> Status: **Revision 7 — ACCEPTED WITH CONDITIONS** after iterative independent review ([responses](docs/review-responses/)). **Gate 0 is closed** — the split-process spike is implemented, its two gating tests pass, and every closure condition from the second review round is verified merged ([evidence](docs/evidence/gate-0.md)); remaining conditions are bound to later gate exits in [roadmap.md](docs/roadmap.md). **Gate 1 is open** — **Gate 1 L00** (versioned Broker RPC v2 contract), **L01** (durable transaction core and read surface), **L02** (hermetic Nix proposal and promotion primitives: verified probe/safety reporting, protected/Class-F rejection, immutable proposal capture, and fake-runner-tested build, one test activation, pin, profile, boot, flush, and agreement-readback boundaries), and **L03** (hermetic watchdog-only decision authority, durable local broker↔watchdog protocol, leases, epoch/safe-mode failure handling, and fencing-ordering contract) are complete ([L00 result](plans/AGB-2/RESULT.md), merged as PR #17; [L01 result](plans/AGB-4/RESULT.md), merged as PR #20; [L02 result](plans/AGB-6/RESULT.md), merged as PR #22; [L03 result](plans/AGB-8/RESULT.md), merged as PR #24). L01–L03 remain non-effecting: no real/live NixOS or Proxmox mutation, no live `nixos-rebuild`, watchdog daemon, real process fencing, commit/recovery orchestration, OOB observer, approvals, ledger, enforcement, deployment, activation, or Gate 2+ capabilities yet. The architecture is in [ADR-001](docs/adr/ADR-001-agentbed-architecture.md) with normative companions [threat-model.md](docs/threat-model.md), [effects.md](docs/effects.md) and [roadmap.md](docs/roadmap.md).
